...
export class BybitFuturesClient {
    ...
    async getAccountInfo(
        ...
        return this.makeRequest<BybitAccountInfo>(
            "GET",
    ...
    async placeOrder(
        ...
        return this.makeRequest<BybitOrderResponse>(
            "POST",
            "/v5/order/create",
            orderParams
        );
    ...
    async getPositions(symbol?: string): Promise<BybitPosition["list"]> {
        ...
        const positions = await this.makeRequest<BybitPosition>(
            "GET",
            "/v5/position/list",
            params
        );
    ...
    async getPosition(
        symbol: string
    ): Promise<BybitPosition["list"][0] | null> {
     ...
     */
    async fetchBybitFuturesCandles(
        input: IFetchBybitFuturesCandlesInput
    ): Promise<IOHLCData[]> {
        try {
            ...
            const candlePromises = input.pairs.map(async (symbol) => {
                try {
                    ...
                    return {
                        ...
                        closeTime:
                            parseInt(latestCandle[0]) +
                            parseInt(input.interval) * 60 * 1000, // Approximate
