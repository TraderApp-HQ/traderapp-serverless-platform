...
(async () => {
    ...
    console.log("users with trading accounts and balances", { usersAccountsAndBalances: JSON.stringify(users, null, 2) });
})().catch((error) => {
    console.error("Error getting users with trading accounts and balances", { error: error.message });
    process.exit(1);
});