...
// src/clients/BybitClient/index.ts
var BybitClient_exports = {};
__export(BybitClient_exports, {
  BybitFuturesClient: () => BybitFuturesClient
});
module.exports = __toCommonJS(BybitClient_exports);
...
var BybitFuturesClient = class {
   ...
   */
  generateSignature(timestamp, paramStr) {
    const message = timestamp + this.apiKey + this.recvWindow + paramStr;
    return import_crypto2.default.createHmac("sha256", this.apiSecret).update(message).digest("hex");
  }
  /**
  ...
  async makeRequest(method, endpoint, params = {}) {
    ...
    try {
      ...
      if (error.response) {
        throw new Error(
          `Bybit API Error: ${error.response.data?.retMsg || error.message}`
        );
      }
   ...
   */
  async getUserInfo() {
    return this.makeRequest("GET", "/v5/user/query-api", {});
  }
  /**
  ...
  async placeOrder(params) {
    ...
    if (params.reduceOnly !== void 0) {
      orderParams.reduceOnly = params.reduceOnly;
    }
    if (params.closeOnTrigger !== void 0) {
   ...
   */
  async fetchBybitFuturesCandles(input) {
    try {
      const candlePromises = input.pairs.map(async (symbol) => {
        try {
          ...
          return {
            ...
            closeTime: parseInt(latestCandle[0]) + parseInt(input.interval) * 60 * 1e3,
            // Approximate
      ...
      throw new Error(
        `Failed to fetch Bybit futures candles: ${error.message}`
      );
    }
  }
  /**
  ...
  async getOrderById({
    ...
    try {
      ...
      const openOrders = await this.makeRequest(
        "GET",
        "/v5/order/realtime",
        openOrderParams
      );
      ...
      const orderHistory = await this.makeRequest(
        "GET",
        "/v5/order/history",
        historyParams
      );
