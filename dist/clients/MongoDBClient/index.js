...
var MongoDBClient = class {
  ...
  async findOne(filter, options) {
    ...
    return this.connection.collection(this.collection).findOne(filter, options);
  }
  async find(filter, options) {
    await this.connection.asPromise();
    const result = this.connection.collection(this.collection).find(filter, options);
  ...
  async insertOne(doc, options) {
    await this.connection.asPromise();
    const result = await this.connection.collection(this.collection).insertOne(doc, options);
    return { ...doc, _id: result.insertedId };
  }
  async updateOne(filter, update, options) {
    await this.connection.asPromise();
    const result = await this.connection.collection(this.collection).updateOne(filter, update, options);
    return {
  ...
  async findOneAndUpdate(filter, update) {
    await this.connection.asPromise();
    const result = await this.connection.collection(this.collection).findOneAndUpdate(
      filter,
      update,
